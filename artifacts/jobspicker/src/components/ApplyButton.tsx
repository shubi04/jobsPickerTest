...
export function ApplyButton({ jobId, jobTitle, onApplied }: ApplyButtonProps) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg rounded-lg border border-border bg-card shadow-lg">
            <div className="flex items-center justify-between border-b border-border px-6 py-4">
              ...
              <button onClick={() => setIsOpen(false)} className="rounded-md p-1 text-muted-foreground hover:bg-muted">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <form onSubmit={handleApply} className="p-6 space-y-4">
              ...
              <div className="flex gap-3 justify-end">
                ...
                >
                  {isLoading ? <><Loader2 className="h-4 w-4 animate-spin" />Applying...</> : <><Send className="h-4 w-4" />Submit Application</>}
                </button>
              </div>
            </form>
          </div>
